...
class RForest:
    ...
    def loadCSV(self,filename):  # 加载数据，一行行的存入列表
        self.data = []
    ...
    def column_to_float(self):
        self.featLen = len(self.data[0]) - 1
        for data in self.data:
            for column in range(self.featLen):
                data[column] = float(data[column].strip())

    # 将数据集随机分成N块，方便交叉验证，其中一块是测试集，其他四块是训练集
    def spiltDataSet(self, n_folds):
        fold_size = int(len(self.data) / n_folds)
    ...
    # 构造数据子集
    def get_subsample(self, ratio):
        subdataSet = []
        lenSubdata = round(len(self.data) * ratio)  # 返回浮点数
        while len(subdataSet) < lenSubdata:
            index = randrange(len(self.data) - 1)
            subdataSet.append(self.data[index])
        # print len(subdataSet)
        return subdataSet

    # 分割数据集
    def data_spilt(self, index, value):
        left = []
        right = []
    ...
    def get_best_spilt(self, n_features):
        features = []
        class_values = list(set(row[-1] for row in self.data))
        b_index, b_value, b_loss, b_left, b_right = 999, 999, 999, None, None
        while len(features) < n_features:
            index = randrange(len(self.data[0]) - 1)
            if index not in features:
                features.append(index)
        # print 'features:',features
        for index in features:  # 找到列的最适合做节点的索引，（损失最小）
            for row in self.data:
                left, right = self.data_spilt(self.data, index, row[index])  # 以它为节点的，左右分支
    ...
    def build_tree(self, n_features, max_depth, min_size):
        root = self.get_best_spilt(self.data, n_features)
    ...
    # 创建随机森林
    def random_forest(self,train, test, ratio, n_feature, max_depth, min_size, n_trees):
        trees = []
        for i in range(n_trees):
            train = self.get_subsample(train, ratio)  # 从切割的数据集中选取子集
            tree = self.build_tree(train, self.n_features, max_depth, min_size)
            # print 'tree %d: '%i,tree
            trees.append(tree)
...
#     seed(1)
#     dataSet = loadCSV('D:/pythonProject1/XXQ/Iris.csv')
